...
export const ingestSales = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    if (!b) {
      return {
        fips: data.county_fips,
        status: "SKIP",
        note: "no sales adapter for this county",
        inserted: 0,
      };
    }
    ...
    if (rows.length && status === "OK") {
      ...
      for (let i = 0; i < stamped.length; i += CHUNK) {
        const { error } = await supabase
          .from("sales")
          .upsert(stamped.slice(i, i + CHUNK), {
            onConflict: "county_fips,external_apn,sold_at,sale_price",
          });
        if (error) {
          status = "PARTIAL";
    ...
    return {
      ...
      fetched: rows.length,
      inserted,
      matched_to_parcels: matched,
      status,
      note,
    };
  });

export const ingestAllNycSales = createServerFn({ method: "POST" })
  .middleware([requireAdmin])
  .handler(async () => {
    ...
    if (rows.length) {
      ...
      for (let i = 0; i < apns.length; i += CHUNK) {
        const { data: matches } = await supabase
          .from("parcels")
          .select("id, apn, county_fips")
          .in(
            "county_fips",
            Object.values({ MN: "36061", BX: "36005", BK: "36047", QN: "36081", SI: "36085" }),
          )
          .in("apn", apns.slice(i, i + CHUNK));
        for (const m of matches ?? []) apnToParcel.set((m as any).apn, (m as any).id);
      }
    ...
    const byBorough: Record<
      string,
      { fetched: number; inserted: number; name: string; fips: string; matched: number }
    > = {};
    for (const b of nycBoroughs())
      byBorough[b.fips] = { fetched: 0, inserted: 0, name: b.name, fips: b.fips, matched: 0 };
