/**
 * Plain (non-server-fn) ingestion helpers. Shared by admin server functions
 * (`src/lib/ingest.functions.ts`) and the cron endpoint
 * (`/api/public/ingest-all`). Uses supabaseAdmin — callers must gate access.
 */
...
export async function ingestCountyCore(args: IngestArgs): Promise<IngestResult> {
  const max = args.max_parcels ?? 5000;
  const enrichFlood = args.enrich_flood ?? true;
  const src = COUNTY_SOURCES.find((s) => s.fips === args.county_fips);
  if (!src) throw new Error(`Unknown county ${args.county_fips}`);
