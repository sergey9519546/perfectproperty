...
function HomePage() {
  ...
  return (
    <>
      <div className="grid min-h-[calc(100vh-3.5rem)] grid-cols-1 grid-rows-[minmax(22rem,45vh)_minmax(24rem,55vh)] gap-0 lg:h-[calc(100vh-3.5rem)] lg:min-h-0 lg:grid-cols-[380px_1fr] lg:grid-rows-1">
        ...
        <div className="flex flex-col overflow-hidden border-r border-border bg-surface/60">
          ...
          <div className="flex-1 overflow-y-auto">
            {q.isLoading && (
              <div className="p-6 text-sm text-muted-foreground">Loading the ranked map…</div>
            )}
        ...
        <div className="relative">
          ...
          <div className="absolute bottom-3 left-3">
            <DataFreshness timestamp={new Date()} prefix="Data" />
          </div>
        </div>
      </div>
...
function CoverageStrip({ cov }: { cov: Awaited<ReturnType<typeof getCoverage>> | undefined }) {
  if (!cov)
    return (
      <div className="border-b border-border px-5 py-4 text-[12px] text-muted-foreground">
        Loading coverage…
      </div>
    );
...
function FilterBar({
  ...
  setRing: (r: number | undefined) => void;
  county: string | undefined;
  setCounty: (c: string | undefined) => void;
          ...
          return (
            <li key={r.parcel_id}>
              <button
                ...
                className={
                  "flex w-full items-start justify-between gap-3 border-b border-border/60 px-5 py-3 text-left transition-colors hover:bg-surface-2 " +
                  (isSel ? "bg-surface-2" : "")
                }
              >
                <div className="min-w-0">
                  ...
                  <div className="mt-0.5 truncate text-[12px] text-muted-foreground">
                    {p.city}, {p.state} · {p.living_sqft?.toLocaleString()} sqft · condition{" "}
                    {p.condition_grade}
                  </div>
                  <div className="mt-1.5 flex flex-wrap items-center gap-x-2 gap-y-1 text-[11px]">
                    <span className="rounded-full border border-border bg-surface-2 px-1.5 py-0.5 text-muted-foreground">
                      {ringLabel(r.ring)}
                    </span>
                    ...
                    <span className="text-muted-foreground">
                      <span className="num">{r.exit_days}d</span> to sell
                    </span>
                  </div>
                </div>
                <div className="flex-shrink-0 text-right">
                  ...
                  <div className="num mt-1 text-[12px] text-profit-strong">
                    +{fmt$(Number(r.gross_profit))}
                  </div>
                  <div className="num text-[11px] text-muted-foreground">
                    offer {fmt$(Number(r.modeled_offer))}
                  </div>
                </div>
              </button>
            </li>
