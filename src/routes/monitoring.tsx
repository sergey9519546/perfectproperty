...
const getLatestMetrics = createServerFn({ method: "GET" }).handler(async () => {
...
export const Route = createFileRoute("/monitoring")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Portfolio-level expected loss, VaR, CVaR, concentration, calibration and risk-appetite breach status." },
    ],
  }),
    ...
    <div className="mx-auto max-w-[1400px] px-6 py-8">
      <div>
        ...
        <p className="mt-1 max-w-3xl text-sm text-muted-foreground">
          Nightly snapshot of portfolio-level expected loss, tail risk, concentration, calibration and risk-appetite budget.
        </p>
      </div>
      ...
      {latest && (
        <>
          <div className="mt-4 text-[11px] text-muted-foreground">
            Latest snapshot <span className="num text-foreground">{new Date(latest.computed_at).toLocaleString()}</span>
            {" · "}
          ...
          {latest.risk_appetite_breached && (
            <div className="mt-4 rounded-lg border border-skeptic/40 bg-skeptic/10 p-4">
              <div className="text-[11px] uppercase tracking-widest text-skeptic">Risk appetite breached</div>
              <div className="mt-2 flex flex-wrap gap-2 text-[12px]">
                {(latest.breach_reasons as string[]).map((r) => (
                  <span key={r} className="rounded-full border border-skeptic/40 bg-skeptic/15 px-2 py-0.5 text-skeptic">{r}</span>
                ))}
              </div>
            </div>
          )}
          ...
          <div className="mt-6 grid grid-cols-2 gap-3 md:grid-cols-4">
            ...
            <Metric
              label="PSI"
              v={<span>{Number(latest.psi ?? 0).toFixed(3)} <span className="ml-1 text-[10px] uppercase" style={{ color: bandColor(latest.psi_band) }}>{latest.psi_band}</span></span>}
            />
            <Metric label="Calib. slope" v={Number(latest.calibration_slope ?? 1).toFixed(2)} tone={latest.calibration_flag ? "skeptic" : undefined} />
            <Metric label="Calib. intercept" v={Number(latest.calibration_intercept ?? 0).toFixed(2)} />
            <Metric label="Deals" v={<span className="num">{latest.n_deals}</span>} />
          </div>

          <div className="mt-8">
            <div className="text-[10px] uppercase tracking-widest text-muted-foreground">History (last 30)</div>
                ...
                <tbody>
                  {history.map((h) => (
                    <tr key={h.id} className="border-t border-border">
                      <td className="num px-3 py-2 text-muted-foreground">{new Date(h.computed_at).toLocaleString()}</td>
                      ...
                      <td className="num px-3 py-2 text-right">{Number(h.hhi_county ?? 0).toFixed(3)}</td>
                      <td className="num px-3 py-2 text-right" style={{ color: bandColor(h.psi_band) }}>{Number(h.psi ?? 0).toFixed(3)}</td>
                      <td className="px-3 py-2 text-[11px]" style={{ color: h.risk_appetite_breached ? "var(--skeptic)" : "var(--muted-foreground)" }}>
...
function Metric({ label, v, tone }: { label: string; v: React.ReactNode; tone?: "skeptic" | "opportunity" }) {
  const color = tone === "skeptic" ? "var(--skeptic)" : tone === "opportunity" ? "var(--opportunity)" : undefined;
  return (
    <div className="rounded-lg border border-border bg-surface p-4">
      <div className="text-[10px] uppercase tracking-widest text-muted-foreground">{label}</div>
      <div className="num mt-1 text-xl font-semibold" style={{ color }}>{v}</div>
    </div>
  );
}
...
function bandColor(band: string | null): string {
  switch (band) {
    case "green": return "var(--profit-strong)";
    case "yellow": return "var(--opportunity)";
    case "orange": return "var(--opportunity)";
    case "red": return "var(--skeptic)";
